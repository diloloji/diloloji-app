...
function ConjugationWithStemSuffix({
  ...
  return (
    <span className="text-slate-900 dark:text-slate-100 font-medium">
      {stemRuns.map((r, i) =>
        r.highlight ? (
          <span key={i} className="font-bold text-orange-600 dark:text-orange-400">
            {r.char}
          </span>
        ) : (
...
/** Zaman çizelgesi tipi: tense id → görsel mantık */
type TimelineVariant = 'future' | 'imperfect' | 'definite-past' | 'present' | 'subjunctive';

function getTimelineVariant(tenseId: string): TimelineVariant {
  const futureIds = ['futur-simple', 'futuro', 'condicional'];
  const imperfectIds = ['imparfait', 'imperfecto'];
  const definitePastIds = ['passe-compose', 'passe-simple', 'preterito'];
  const subjunctiveIds = ['subjonctif-present', 'subjuntivo-presente'];
  if (futureIds.includes(tenseId)) return 'future';
  if (imperfectIds.includes(tenseId)) return 'imperfect';
  if (definitePastIds.includes(tenseId)) return 'definite-past';
  if (subjunctiveIds.includes(tenseId)) return 'subjunctive';
  return 'present';
}

const TIMELINE_DESCRIPTIONS: Record<TimelineVariant, string> = {
  future: 'Gelecekte gerçekleşecek eylem',
  imperfect: 'Geçmişte süren eylem veya alışkanlık',
  'definite-past': 'Geçmişte tamamlanmış eylem',
  present: 'Şu an veya genel gerçek',
  subjunctive: 'Dilek, olasılık veya bağımlı ifade',
};
...
/** Fiil Laboratuvarı: Seçilen zamanın mantığını görselleştiren yatay çizelge (Şimdi ortada). */
function VisualTimeline({ tenseId }: { tenseId: string }) {
  const variant = getTimelineVariant(tenseId);
  const description = TIMELINE_DESCRIPTIONS[variant];

  return (
    <div className="mx-4 sm:mx-0 mb-6 rounded-2xl border border-slate-200/80 dark:border-slate-600/80 bg-slate-50/50 dark:bg-slate-800/30 backdrop-blur-sm overflow-hidden">
      <div className="px-4 py-4 sm:px-6">
        <div className="relative flex items-center justify-center" style={{ minHeight: '3rem' }}>
          {/* Yatay eksen: ince slate-700 çizgisi, ortada Şimdi için boşluk */}
          <div className="absolute left-0 top-1/2 h-px w-[calc(50%-1.5rem)] -translate-y-1/2 bg-slate-700 dark:bg-slate-600" aria-hidden />
          <div className="absolute right-0 top-1/2 h-px left-[calc(50%+1.5rem)] -translate-y-1/2 bg-slate-700 dark:bg-slate-600" aria-hidden />

          {/* Ortadaki Şimdi noktası */}
          <div className="relative z-10 flex flex-col items-center gap-1 px-2">
            <div className="w-2.5 h-2.5 rounded-full bg-slate-500 dark:bg-slate-400 ring-2 ring-slate-300 dark:ring-slate-500 shrink-0" />
            <span className="text-[10px] sm:text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">Şimdi</span>
          </div>

          {/* Gelecek: sağda parlayan nokta + çizgi vurgusu */}
          {variant === 'future' && (
            <>
              <div className="absolute left-1/2 right-6 top-1/2 h-px w-[calc(50%-2rem)] -translate-y-1/2 bg-gradient-to-r from-blue-500/50 to-blue-500 dark:from-indigo-400/50 dark:to-indigo-400" style={{ marginLeft: '1rem' }} aria-hidden />
              <div className="absolute right-6 top-1/2 h-3 w-3 -translate-y-1/2 rounded-full bg-blue-500 dark:bg-indigo-400 shadow-[0_0_14px_4px] shadow-blue-500/50 dark:shadow-indigo-400/50 ring-2 ring-blue-400/50 dark:ring-indigo-300/50" aria-hidden />
            </>
          )}

          {/* Imparfait: solda tırtıklı (dashed) süreç çizgisi */}
          {variant === 'imperfect' && (
            <div className="absolute left-6 right-1/2 top-1/2 -translate-y-1/2 flex items-center" style={{ marginRight: '1rem' }}>
              <div className="h-px w-full border-t-2 border-blue-500 dark:border-indigo-400 border-dashed opacity-90" aria-hidden />
            </div>
          )}

          {/* Belirli geçmiş: solda tek, net nokta */}
          {variant === 'definite-past' && (
            <>
              <div className="absolute left-6 right-1/2 top-1/2 h-px w-[calc(50%-2.5rem)] -translate-y-1/2 bg-blue-500/50 dark:bg-indigo-400/50" style={{ marginRight: '1rem' }} aria-hidden />
              <div className="absolute left-6 top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-blue-500 dark:bg-indigo-400 shadow-[0_0_10px] shadow-blue-500/40 dark:shadow-indigo-400/40 ring-2 ring-blue-400/40 dark:ring-indigo-300/40" aria-hidden />
            </>
          )}

          {/* Present: ortada hafif mavi/indigo glow */}
          {variant === 'present' && (
            <div className="absolute left-1/2 top-1/2 h-2 w-10 -translate-x-1/2 -translate-y-1/2 rounded-full bg-blue-500/30 dark:bg-indigo-400/30" aria-hidden />
          )}

          {/* Subjunctive: ortada hafif vurgu */}
          {variant === 'subjunctive' && (
            <div className="absolute left-1/2 top-1/2 h-1.5 w-8 -translate-x-1/2 -translate-y-1/2 rounded-full bg-indigo-400/25 dark:bg-indigo-300/25" aria-hidden />
          )}
        </div>
...
export function Page() {
  ...
  /** Zamana Karşı modu: süre (saniye), soru, cevap, skor, oyun bitti mi */
  ...
  const submitTimeAttackAnswer = useCallback(() => {
    ...
    if (isCorrect) {
      ...
      setTimeAttackTimeLeft((t) => Math.max(0, t - 5));
    }
    setTimeAttackInput('');
    setTimeAttackQuestion(getRandomTimeAttackQuestion(selectedLanguage));
    setTimeout(() => setTimeAttackFeedback(null), 800);
  }, [timeAttackQuestion, timeAttackGameOver, timeAttackInput]);
  ...
  // Klavye kısayolları: Alt+L Learning, Alt+Q Quiz, Escape quiz temizle / review'dan çık / aktivite modal kapat
  useEffect(() => {
  ...
  }, [mode, verbKey, resetQuiz, showActivityModal]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
      {/* Üst menü (Navbar) — ince, cam efekti, sticky */}
      <header className="w-full flex justify-between items-center py-3 px-4 sm:px-5 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700/50 sticky top-0 z-50 transition-colors duration-300">
        <div className="min-w-0 flex items-center gap-3 flex-1">
          <Link
            to="/"
            className="flex items-center gap-3 shrink-0 transition-all duration-300 hover:opacity-90 hover:scale-[1.02] cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:ring-offset-2 dark:focus:ring-offset-slate-900 rounded-lg"
          ...
          {/* Uygulama modu: Fiil Laboratuvarı / Ezber Makinesi */}
          <div
            className="ml-2 sm:ml-4 flex rounded-xl bg-slate-100 dark:bg-slate-800 p-1 border border-slate-200 dark:border-slate-600 shrink-0"
            role="tablist"
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          {/* Skor alanı: puan • kombo — hap butonların solunda */}
          <button
            type="button"
            onClick={() => setShowActivityModal(true)}
            className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400 tabular-nums hover:text-slate-700 dark:hover:text-slate-300 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:ring-offset-1 dark:focus:ring-offset-slate-900 rounded px-1.5 py-1"
          ...
          {/* Hap butonlar: Zorlandıklarım, Yıldızlı */}
          <button
            type="button"
            onClick={openReviewMode}
            className="flex items-center gap-1.5 rounded-full bg-slate-100 dark:bg-slate-700/80 hover:bg-slate-200 dark:hover:bg-slate-600 px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-200 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:ring-offset-1 dark:focus:ring-offset-slate-900 border border-transparent dark:border-slate-600/50"
            title="Zorlandıklarım"
            aria-label="Zorlandıklarım"
          >
            <span aria-hidden>🧠</span>
            <span className="tabular-nums" aria-live="polite">{dueCount}</span>
          </button>
          <button
            type="button"
          ...
          {/* Ayırıcı + Dil (kapsül) + Tema — sağa yaslı */}
          <span className="border-l border-slate-200 dark:border-slate-700 h-6 mx-1 sm:mx-2 shrink-0" aria-hidden />
          <div className="flex items-center gap-2">
            ...
            {!themeMounted ? (
              <div className="rounded-lg bg-slate-100 dark:bg-slate-700/80 w-8 h-8 shrink-0" aria-hidden />
            ) : (
              <button
                type="button"
                onClick={toggleTheme}
                className="p-1.5 rounded-lg bg-slate-100/80 dark:bg-slate-700/80 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:ring-offset-1 dark:focus:ring-offset-slate-900 shrink-0"
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 pb-20 flex flex-col gap-4">
        ...
        {showCongrats && verbKey && mode === 'quiz' && (
          ...
          >
            ...
            <button
              ...
              className="mt-4 rounded-xl bg-emerald-600 dark:bg-emerald-500 text-white text-sm font-medium px-5 py-2.5 hover:bg-emerald-700 dark:hover:bg-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-400 focus:ring-offset-2 dark:focus:ring-offset-slate-900 transition-colors duration-300"
            >
              Kapat
            </button>
          </div>
        ...
        {mode !== 'review' && mode !== 'starred' && (
          <section className="rounded-2xl bg-white dark:bg-slate-800/80 shadow-sm border border-slate-100 dark:border-slate-700/50 overflow-visible mb-4 backdrop-blur-md transition-colors duration-300 min-h-[400px]">
            ...
            {/* Zamana Karşı: oyun alanı veya oyun bitti ekranı */}
            {mode === 'time-attack' && (
              <div className="p-6 sm:p-8">
                {timeAttackGameOver ? (
                  <div className="text-center">
                    <p className="text-slate-700 dark:text-slate-200 font-semibold text-lg">Süre doldu!</p>
                    <p className="text-slate-600 dark:text-slate-300 mt-2">
                      <span className="font-bold text-indigo-600 dark:text-indigo-400">{timeAttackScore}</span> doğru cevap
                    </p>
                    <button
                      type="button"
                      onClick={restartTimeAttack}
                      className="mt-6 px-6 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white font-medium shadow-sm transition-colors duration-300"
                    >
                      Tekrar Oyna
                    </button>
                  </div>
                  ...
                  <>
                    <div className="flex items-center justify-between mb-6">
                      <span className="text-3xl font-mono font-bold tabular-nums text-slate-800 dark:text-slate-100">
                        {Math.floor(timeAttackTimeLeft / 60)}:{(timeAttackTimeLeft % 60).toString().padStart(2, '0')}
                      </span>
                      <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
                        Doğru: <span className="text-indigo-600 dark:text-indigo-400 font-semibold">{timeAttackScore}</span>
                      </span>
                    </div>
                    {timeAttackQuestion && (
                      <>
                        ...
                        {timeAttackFeedback === 'wrong' && (
                          <p className="text-center mt-3 text-red-600 dark:text-red-400 font-medium">−5 saniye</p>
                        )}
                      </>
                    )}
                  </>
                          ...
                          <li key={id} className="flex items-center justify-between gap-4">
                            ...
                            <span className="text-slate-900 dark:text-slate-100 text-right">
                              <ConjugationWithStemSuffix
                                text={getConjugationForTenseForLang(verbKey, compareTense1, selectedLanguage)[id] ?? '—'}
                      ...
                      <ul className="space-y-3">
                        {pronounsForLang.map(({ id, label }) => (
                          <li key={id} className="flex items-center justify-between gap-4">
                            ...
                            <span className="text-slate-900 dark:text-slate-100 text-right">
                              <ConjugationWithStemSuffix
                                text={getConjugationForTenseForLang(verbKey, compareTense2, selectedLanguage)[id] ?? '—'}
        ...
        {verbKey && mode === 'learning' && conjugationsForDisplay && (
          <>
            ...
            {isIrregularVerb(verbKey, selectedLanguage) && (
              <div className="mx-4 sm:mx-0 mb-4 rounded-xl bg-amber-500/10 dark:bg-orange-500/10 border border-amber-500/20 dark:border-orange-500/20 px-4 py-3 flex items-start gap-3" role="alert">
